(function(window, undefined) {
	...
	var Page	= (function() {
		...
		var $container			= $( '#container' ),
			...
			$menu1				= $container.find( '.menu-link' ),
			// menu links
			$links				= $menu.find( 'nav > a' ),
			$reorder			= $container.find('.reorder'),
			...
			_goto				= function( chapter ) {
				...
				var chapter 	= chapter || History.getState().url.queryStringToJSON().page,
					...
					$('#link_ubbey').removeClass('active');
					$('#link_protocol').removeClass('active');
					$('#link_team').removeClass('active');
					$('#link_plan').removeClass('active');

					console.log(chapter, $article.length)
				...
				if( $article.length ) {
					...
					var left		= $article.position().left,
						...
						param		= ( is_v ) ? { scrollTop : (isHome) ? top : Math.max(0, top - $menu.outerHeight( true )) } : { scrollLeft : left },
